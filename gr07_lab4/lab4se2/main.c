...
/* Configure the peripherals common clocks */
  ...
  LL_GPIO_WriteReg(GPIOE, ODR, LL_GPIO_ReadReg(GPIOE, ODR) & ~(1 << 0));   // porta IO0 settata a 0

  while (1)
  {
	  if ( LL_GPIO_ReadReg(GPIOB, IDR) & (1 << 3) ){	// stato SWITCH 0
	  		LL_GPIO_WriteReg(GPIOC, ODR, LL_GPIO_ReadReg(GPIOC, ODR) | (1 << 10));		// LED0 acceso
	  	}else{
	  		LL_GPIO_WriteReg(GPIOC, ODR, LL_GPIO_ReadReg(GPIOC, ODR) & ~(1 << 10));	// LED0 spento
	  	}
	  	...
	  	if ( LL_GPIO_ReadReg(GPIOB, IDR) & (1 << 5) ){	// stato onda SWITCH 2
	  		LL_TIM_WriteReg(TIM4, CR1, LL_TIM_ReadReg(TIM4, CR1) | (1 << 0));	// attivo contatore
	  	}else{
